    ...
    : QLabel(parent)
{
    ...
    pages[k++] = pageSendCAT = new QPageSendCAT(this);
    pages[k++] = pageReceiveCAT = new QPageAddrBook(QPageAddrBook::ForEditing, QPageAddrBook::ReceivingTab, this);
...
void QStealthMain::setModel(WalletModel *model)
{
    ...
    pageSendCAT->setModel(model);
    pageReceiveCAT->setModel(model->getAddressTableModel());
...
void QStealthMain::showOutOfSyncWarning(bool fShow)
{
    ...
    pageSendCAT->showOutOfSyncWarning(fShow);
