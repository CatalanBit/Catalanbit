...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("CATALANBIT version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  CATALANBITd [options]                     " + "\n" +
                  "  CATALANBITd [options] <command> [params]  " + _("Send command to -server or CATALANBITd") + "\n" +
                  "  CATALANBITd [options] help                " + _("List commands") + "\n" +
                  "  CATALANBITd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "CATALANBIT:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("CATALANBIT"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("CATALANBIT"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: CATALANBIT.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: CATALANBITd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  CATALANBIT is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "CATALANBIT server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("CATALANBIT"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of CATALANBIT") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart CATALANBIT to complete") << "\n";
