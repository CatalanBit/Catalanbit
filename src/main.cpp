...
unsigned int nStakeMinAge = 60 * 60 * 24 * 200;	//minimum age for coin age:  48 h
unsigned int nStakeMaxAge = 60 * 60 * 24 * 240;	//stake age of full weight: 
unsigned int nStakeTargetSpacing = 120;			// 120 sec block spacing

int64 nChainStartTime = 1507746474;
...
const string strMessageMagic = "CATALANBIT Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
        nSubsidy = 18000000 * COIN; // genesis block coinbase is unspendable

    nSubsidy >>= (nHeight / 518400);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CATALANBIT
    bool fStrictPayToScriptHash = true; // Always active in CATALANBIT
...
if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64)vtx[1].nTime))
    ...
    if ((nTime >= CUTOFF_POW_TIME) && (IsProofOfWork()))
         return DoS(100, error("CheckBlock() : Proof of work (%f CATALANBIT) at t=%d on or after %d.\n",
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CATALANBIT", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "20171011 CATALANBIT proves that pzTimestamp is overkill.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1507746474;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 201102; // perfect nonce
        ...
        assert(block.hashMerkleRoot == uint256("0x3c545c588945f54885bc558f3751dbbf94699679886cc3c23fa4e24aba68ca51"));
